...
shinyServer(function(input, output) {
  ...
  df.all.clean.merge_final <- reactive({
    ...
    more.info.select <- more.info %>% 
      select("Code","Unit cost description", "UC source(s)", "PIN description", "PIN source(s)", "Coverage description", "Coverage source(s)")
  ...
  output$raw.table <- DT::renderDataTable(rownames=FALSE, filter="top",
                                          datatable2(
                                            ...
                                            vars = c("Unit cost description", "UC source(s)", "PIN description", "PIN source(s)", "Coverage description", "Coverage source(s)"),
  ...
  data.lmic <- reactive({
    ...
    if (!is.null(hot.lmic)) hot_to_r(hot.lmic)
  })
  ...
  result.user.lic <- reactive({
    ...
    lic.full$diff_cost <- ifelse(lic.full$diff_cost <= 0.00001,
                                 0,
                                 lic.full$diff_cost
                                 )
  ...
  result.user.lmic <- reactive({
    ...
    full.df.lmic$diff_cost <- ifelse(full.df.lmic$diff_cost <= 0.0001,
                                 0,
  ...
  output$result.user.table <- function() {
    result.user.both.sum() %>%
      knitr::kable("html") %>%
      kable_styling("striped", full_width = F) %>%
      add_header_above(c(" ","LICs" = 2, "LMICs" = 2))
  }
  ...
  output$result.figure1 <- renderPlot({
    ...
    grid.arrange(f1,f2, nrow = 1, widths = c(1,0.9))
